...
import random
import pickle
from numpy.core.fromnumeric import _argmax_dispatcher
import torch
import re
        ...
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            ...
            while True:
                ...
                if not line.strip():
                    ...
                    continue
                # 按kmer划分
                text_list = re.findall(".{" + str(self.kmer) + "}", line)
                if len(line) % self.kmer != 0: text_list.append(line[len(line)//self.kmer * self.kmer : ])
                line = " ".join(text_list)
