...
def evaluate(args, dataset):
    ...
    auc = roc_auc_score(y_score=all_prob, y_true=all_gold)
    acc = accuracy_score(y_pred=all_pred, y_true=all_gold)
    mcc = matthews_corrcoef(y_pred=all_pred, y_true=all_gold)

    # sn = confusion[1, 1] / np.sum(confusion[1, :])    # 预测正确的正样本
    # sp = confusion[0, 0] / np.sum(confusion[0, :])    # 预测正确的负样本

    eps = 1e-9
    sn = confusion[1, 1].item() / (confusion[1, :].sum().item() + eps)
    sp = confusion[0, 0].item() / (confusion[0, :].sum().item() + eps)

    print(f"sn: {sn:.4f} - sp: {sp:.4f} - mcc: {mcc:.4f} - auc: {auc:.4f} - acc: {acc:.4f}")
